...
```bash
npx firebase-tools deploy --only hosting   # 取得 → site/ を組み立て → 配信 → 照合
```
...
デプロイの最後に `scripts/verify-deploy.mjs` が走り、**`site/` の全ファイルを本番と
突き合わせます。**「一致しました」が出て、はじめて出せたことになります。

```
本番と照合します（1242 件）… 一致しました（https://kinetone.web.app）
```

食い違えば、どのファイルがどう違うかを並べて終了コード 1 で終わります。

| 出るもの | 意味 |
|---|---|
| `中身が違う` | 本番が古い。もう一度デプロイする |
| `本番に無い` | 置かれていない。rewrites の受け皿が拾って 200 を返している |
| `手元から消えた` | 照合中に `site/` が作り直された。もう一度走らせる |

**`tag` を上げたのにデプロイを忘れた、コミットしたのに出していなかった、という
取り違えはこれで防げます。**画面を見ただけでは、手元では直っているので気づけません。

デプロイせずに、今ずれていないかだけ調べることもできます。

```bash
node scripts/build-site.mjs && node scripts/verify-deploy.mjs
```
...
| [`one-leg-stand`](https://github.com/researchcoordinate/kinetone-one-leg-stand) | `/one-leg-stand/` | 片足立ちテスト（片足で立っていられる時間。**カメラに対して斜めに立つ**） |
