...
WebGPU では 320〜512px で所要時間がほぼ変わらないので、**遠くの人を拾えるよう
入力を大きめに取っている。既定は 640px**（段階2で、遠く・細く写る単独被写体が
448px では検出されず 640px で検出できると分かったため引き上げた。640px は上表の
延長で内蔵GPU 推論 ≈27〜31ms・24fps 追従。詳細は `docs/2026-07-27-段階2-作業記録.md` 3.5 節）。
WebGL に落ちた機械では 640px は重すぎるので 192〜320 へ下げること。

### 連続稼働（11分）

テスト動画（24fps）を入力に、WebGPU / 448px（当時の既定。現在の既定は 640px）で連続稼働:
